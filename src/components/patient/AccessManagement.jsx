...
function AccessManagement({ apiBase }) {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const recordRes = await fetch(`${apiBase}/api/patient/records/pending-access`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const records = recordRes.ok ? await recordRes.json() : [];

      const historyRes = await fetch(`${apiBase}/api/patient/history/requests`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const approveRecord = async (accessId) => {
    try {
      const res = await fetch(`${apiBase}/api/patient/records/access/${accessId}/approve`, {
        method: "POST",
        headers: {
  ...
  const rejectRecord = async (accessId) => {
    try {
      const res = await fetch(`${apiBase}/api/patient/records/access/${accessId}/reject`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  const approveHistory = async (requestId) => {
    try {
      const res = await fetch(`${apiBase}/api/patient/history/requests/${requestId}/approve`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const rejectHistory = async (requestId) => {
    try {
      const res = await fetch(`${apiBase}/api/patient/history/requests/${requestId}/reject`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
