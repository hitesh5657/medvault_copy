...
function PatientMedicalRecords({ apiBase }) {
  ...
  const fetchRecords = () => {
    fetch(`${apiBase}/api/patient/records`, {
  ...
  const handleDelete = (recordId) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`${apiBase}/api/patient/records/${recordId}`, {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => res.text())
  ...
  const openRecordModal = async (record) => {
    try {
      const res = await fetch(`${apiBase}/api/patient/records/${record.id}/view`, {
        headers: { Authorization: `Bearer ${token}` },
      });
