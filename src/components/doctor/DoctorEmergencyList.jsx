...
function DoctorEmergencyList({ apiBase }) {
  ...
  useEffect(() => {
    const fetchEmergencies = async () => {
      try {
        const res = await fetch(`${apiBase}/api/doctor/emergencies`, {
    ...
    const fetchAcceptedEmergencies = async () => {
      try {
        const res = await fetch(`${apiBase}/api/doctor/emergencies/accepted`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleAccept = async (emergencyId) => {
    try {
      const res = await fetch(`${apiBase}/api/doctor/emergency/accept/${emergencyId}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
